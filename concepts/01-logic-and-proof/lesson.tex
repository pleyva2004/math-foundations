...
Before any formal apparatus, look at one truth table and one short proof.

\emph{Truth table for $p \Rightarrow q$.} The implication ``if $p$ then $q$''
is false only in the single case where the hypothesis $p$ is true and the
...
\emph{Three-step direct proof: the sum of two even integers is even.}
\begin{enumerate}
  \item Let $m = 2a$ and $n = 2b$ for some integers $a, b$ (definition of even).
  \item Then $m + n = 2a + 2b = 2(a+b)$ (factor).
  \item Since $a+b$ is an integer, $m+n$ is twice an integer, hence even.
\end{enumerate}
That is the entire skeleton of a direct proof: unfold the hypothesis, do
algebra, recognise the conclusion. The rest of this lesson formalises the
vocabulary and adds three more strategies.
...
\end{align*}
\emph{Read aloud:} ``not $p$'' is true exactly when $p$ is false; ``$p$ and $q$''
needs both true; ``$p$ or $q$'' needs at least one true; ``$p$ implies $q$''
fails only when $p$ is true and $q$ is false; ``$p$ iff $q$'' holds when
$p$ and $q$ share the same truth value.
\end{definition}
...
\]
\emph{Read aloud:} ``the sum, as $k$ runs from $1$ to $n$, of $k$, equals
$n$ times $n$ plus one, all over two.''
\end{theorem}
\begin{proof}
...
\]
\emph{Read aloud:} ``the sum, as $k$ runs from $1$ to $m$, of $k$, equals
$m$ times $m$ plus one, over two --- this is the inductive hypothesis.''
...
\end{align*}
\emph{Read aloud:} ``the sum from $k=1$ to $m+1$ equals the sum from $k=1$ to
$m$ plus the new term $(m+1)$; substitute the inductive hypothesis; factor out
$(m+1)$; simplify; recognise the closed form at $n = m+1$.''
