...
f(x,y) = x^2 y + \sin(xy).
\]
\emph{Read aloud:} f of x and y equals x squared times y plus sine of x y.
Holding $y$ fixed and differentiating in $x$, then holding $x$ fixed and differentiating in $y$:
\[
\frac{\partial f}{\partial x} = 2xy + y\cos(xy), \qquad \frac{\partial f}{\partial y} = x^2 + x\cos(xy).
\]
\emph{Read aloud:} partial f by partial x equals two x y plus y cosine of x y; partial f by partial y equals x squared plus x cosine of x y.
At $(x,y) = (1,2)$ this gives $\partial_x f(1,2) = 4 + 2\cos 2$ and $\partial_y f(1,2) = 1 + \cos 2$, so the gradient is $\grad f(1,2) = (4 + 2\cos 2,\; 1 + \cos 2)$. Keep this concrete pair of numbers in mind as the abstract machinery unfolds.
...
\]
\emph{Read aloud:} partial f by partial x sub i at x is defined as the limit, as h goes to zero, of f of x plus h e sub i, minus f of x, all over h.
...
\]
\emph{Read aloud:} nabla f at x is defined as the tuple whose i-th entry is partial f by partial x sub i at x, an element of R to the n.
\end{definition}
...
\]
\emph{Read aloud:} D sub v of f at x is defined as the limit, as h goes to zero, of f of x plus h v, minus f of x, all over h.
\end{definition}
...
\lim_{h \to 0} \frac{\norm{f(x+h) - f(x) - L h}}{\norm{h}} = 0.
\]
\emph{Read aloud:} the limit, as h goes to zero, of the norm of f of x plus h minus f of x minus L h, divided by the norm of h, equals zero.
...
\]
\emph{Read aloud:} the derivative of f composed with gamma at t-naught equals the gradient of f at a dotted with gamma-prime at t-naught, which equals the sum from i equals one to n of partial f by partial x sub i at a times gamma sub i prime at t-naught.
\end{theorem}
...
\end{equation}
\emph{Read aloud:} norm of w less than delta implies the absolute value of f of a plus w minus f of a minus L w is at most epsilon times the norm of w.
...
\end{equation}
\emph{Read aloud:} f of a plus w minus f of a equals L w plus phi of w times the norm of w, with phi of zero defined to be zero.
...
\end{equation}
\emph{Read aloud:} gamma of t-naught plus s minus gamma of t-naught equals s times gamma-prime of t-naught plus s times psi of s, with psi of zero defined to be zero.

Set $w(s) := \gamma(t_0 + s) - a$. By~\eqref{eq:diffGamma}, $w(s) = s(\gamma'(t_0) + \psi(s))$, so
\begin{equation}\label{eq:wbound}
\norm{w(s)} \le |s| \bigl( \norm{\gamma'(t_0)} + \norm{\psi(s)} \bigr).
\end{equation}
\emph{Read aloud:} the norm of w of s is at most the absolute value of s times the quantity norm of gamma-prime of t-naught plus norm of psi of s.
...
\]
\emph{Read aloud:} f of gamma of t-naught plus s minus f of a equals L applied to w of s, plus phi of w of s times the norm of w of s.
By linearity of $L$ and~\eqref{eq:diffGamma},
\[
L w(s) = L\bigl( s \gamma'(t_0) + s \psi(s) \bigr) = s \, L \gamma'(t_0) + s \, L \psi(s).
\]
\emph{Read aloud:} L applied to w of s equals L applied to s times gamma-prime of t-naught plus s times psi of s, which equals s times L of gamma-prime of t-naught plus s times L of psi of s.
Therefore
\begin{equation}\label{eq:combine}
\frac{f(\gamma(t_0+s)) - f(\gamma(t_0))}{s} = L \gamma'(t_0) + L \psi(s) + \varphi(w(s)) \cdot \frac{\norm{w(s)}}{s}.
\end{equation}
\emph{Read aloud:} the difference quotient, f of gamma of t-naught plus s minus f of gamma of t-naught, divided by s, equals L of gamma-prime of t-naught plus L of psi of s plus phi of w of s times the norm of w of s over s.
...
\]
\emph{Read aloud:} D sub v of f at x equals the derivative of f composed with gamma at zero, which equals the gradient of f at x dotted with v.
...
\]
\emph{Read aloud:} D sub v of f at one comma two equals four plus two cosine two plus one plus cosine two, all divided by root two, which equals five plus three cosine two over root two.
