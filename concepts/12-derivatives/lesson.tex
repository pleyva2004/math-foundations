...
Before any definitions, let us compute a derivative by hand. Take
$f(x) = x^2$ and ask: what is $f'(2)$? Working straight from the
limit-of-the-difference-quotient idea,
\[
f'(2) \;=\; \lim_{h \to 0} \frac{(2+h)^2 - 4}{h}
       ...
       \;=\; 4.
\]
\emph{Read aloud:} ``f-prime of two equals the limit as h goes to zero of two plus h, squared, minus four, over h; expanding gives four plus four h plus h-squared minus four, over h; the constants cancel and dividing by h leaves four plus h, whose limit as h tends to zero is four.''
So the slope of $y = x^2$ at the point $(2, 4)$ is $4$. The rest of the
lesson formalises what ``slope at a point'' means and what we are
...
\]
\emph{Read aloud:} ``f-prime of c is defined as the limit as h tends to zero of f of c plus h minus f of c, all over h.''
...
\]
\emph{Read aloud:} ``f-prime of c equals the limit as x approaches c of f of x minus f of c, all over x minus c.''
\end{definition}
...
\]
\emph{Read aloud:} ``f of c plus h equals f of c, plus m times h, plus h times epsilon of h.''
\end{definition}
...
\end{equation}
\emph{Read aloud:} ``f of x minus f of c equals the difference quotient, f of x minus f of c over x minus c, times the quantity x minus c.''
By the definition of $f'(c)$,
\[
\lim_{x \to c} \frac{f(x) - f(c)}{x - c} \;=\; f'(c) \;\in\; \mathbb{R}.
\]
\emph{Read aloud:} ``the limit as x approaches c of f of x minus f of c, all over x minus c, equals f-prime of c, which lies in the real numbers.''
Also, trivially,
\[
\lim_{x \to c} (x - c) \;=\; 0.
\]
\emph{Read aloud:} ``the limit as x approaches c of x minus c equals zero.''
...
\;=\; f'(c) \cdot 0 \;=\; 0.
\]
\emph{Read aloud:} ``the limit as x approaches c of f of x minus f of c equals the limit of the difference quotient times the limit of x minus c, which is f-prime of c times zero, which is zero.''
...
\]
\emph{Read aloud:} ``f-prime of xi equals f of b minus f of a, all over b minus a.''
\end{theorem}
...
\]
\emph{Read aloud:} ``f of c plus h, minus f of c, all over h, equals the quantity c plus h squared minus c squared, over h; equals two c h plus h squared, over h; equals two c plus h, which tends to two c as h goes to zero.''
