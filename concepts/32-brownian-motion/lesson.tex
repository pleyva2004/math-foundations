...
Before the abstract definition, build $W_t$ by hand. Fix a horizon
$T = 1$ and a step $\Delta t = 1/N$ for some large $N$. Draw iid
standard Gaussians $Z_1, \dots, Z_N \sim \mathcal{N}(0,1)$ and define
$W_0 = 0$ and recursively
$$W_{(k+1)\Delta t} = W_{k\Delta t} + \sqrt{\Delta t}\, Z_k.$$
\emph{Read aloud:} W at time k plus one delta t equals W at time k delta t plus square root of delta t times Z sub k.
By the sum-of-Gaussians rule, at any fixed grid time $t = k\Delta t$,
$W_t \sim \mathcal{N}(0, t)$, so $\Var(W_t) = t$ and
$\Cov(W_s, W_t) = \min(s, t)$. Letting $N \to \infty$ (with linear
interpolation between grid points) gives a continuous random function
on $[0,1]$ — the sample path of Brownian motion. These paths are
continuous but \emph{nowhere differentiable}: each $\sqrt{\Delta t}$
increment is huge compared to $\Delta t$, so the path wiggles violently
on every scale.
...
$$\E[W_s W_t] = \min(s, t).$$
\emph{Read aloud:} expectation of W sub s times W sub t equals minimum of s and t.
...
Write
$$W_t = W_s + (W_t - W_s),$$
\emph{Read aloud:} W sub t equals W sub s plus the increment W sub t minus W sub s.
which is valid by definition. Substituting into the product gives
$$W_s W_t = W_s \big( W_s + (W_t - W_s) \big) = W_s^2 + W_s(W_t - W_s).$$
\emph{Read aloud:} W sub s times W sub t equals W sub s squared plus W sub s times the increment W sub t minus W sub s.
Taking expectations and using linearity,
\begin{equation}\label{eq:split}
  \E[W_s W_t] = \E[W_s^2] + \E\big[W_s (W_t - W_s)\big].
\end{equation}
\emph{Read aloud:} expectation of W sub s times W sub t equals expectation of W sub s squared plus expectation of W sub s times the increment.
...
\emph{Read aloud:} expectation of W sub s squared equals variance of W sub s plus the square of its mean, which is s plus zero equals s.
...
\emph{Read aloud:} expectation of W sub s times the increment equals expectation of W sub s times expectation of the increment, which is zero times zero equals zero.
...
\emph{Read aloud:} expectation of W sub s times W sub t equals s plus zero equals s equals minimum of s and t.
